...
app = FastAPI(title="API Calidad del Aire - Rango de Fechas")
...
# Nuevo esquema que acepta rangos
...
def predecir_rango_calidad(payload: RangePredictRequest):
    try:
        ...
        return {"error": "Formatos de fecha inválidos. Use AAAA-MM-DD."}
    ...
    resultados_rango = []
    current_dt = start_dt
    max_dias = 15  # Límite seguro para la simulación gráfica
    dias_procesados = 0

    while current_dt <= end_dt and dias_processed < max_dias:
        dia = current_dt.day
        mes_obj = current_dt.month
        
        # Factor de variación dinámico por cada día del rango
        factor = (dia * 0.05) - 0.4
